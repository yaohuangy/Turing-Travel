"""End-to-end test: full user flow (generate → save → list → export → delete)."""
...
MOCK_POI = {
    "name": "西湖",
    "address": "浙江省杭州市西湖区",
    "location": {"lng": 120.141, "lat": 30.238},
    "poi_id": "B0FFG9XXXX",
}
...
MOCK_WEATHER = [
    {"date": "2026-06-01", "condition": "晴", "temp_high": 28, "temp_low": 18, "wind": "东南 ≤3"},
]
            ...
            with patch("app.services.map_service.route_driving", new_callable=AsyncMock) as mock_route:
                with patch("app.services.weather_service.get_forecast", new_callable=AsyncMock) as mock_weather:
                    ...
                    async with AsyncClient(transport=transport, base_url="http://test") as client:
                        ...
                        resp = await client.post("/api/trip/generate", json=gen_body)
                        ...
                        resp = await client.post("/api/trip/save", json=save_body)
                        assert resp.status_code == 200
                        ...
                        # 4. Get trip detail
                        resp = await client.get(f"/api/trip/trip/{trip_id}")
                        assert resp.status_code == 200
                        detail = resp.json()
                        ...
                        # 7. Delete trip
                        resp = await client.delete(f"/api/trip/trip/{trip_id}")
                        assert resp.status_code == 200
